...
#------------------------------------------------------------------------
def OnMouseSelectStarted(pos):
    """
    Triggers the exact millisecond the user clicks down to drag a selection.
    Sets the anchor point for Vim's visual mode block tracking.
    """
    global g_Mode
    global g_VisualModeStartPos
    
    # If we aren't already handling a multi-cursor shortcut or suspended state
    if g_Mode != Mode.SUSPENDED:
        g_VisualModeStartPos = pos
        # Optional: transition instantly or wait until mouse up
    return False # Return False so 10x continues its native drag selection

#------------------------------------------------------------------------
def OnMouseSelectFinished(pos):
    """
    Triggers when the user releases the left mouse button.
    Reads what 10x visually highlighted and syncs it natively to Vim's state.
    """
    global g_Mode
    global g_VisualModeStartPos
    ...
    # Check if a selection actually exists (ruling out a single click)
    start_sel, end_sel = N10X.Editor.GetSelectionStart(), N10X.Editor.GetSelectionEnd()
    
    if start_sel != end_sel:
        # 1. Update the internal Vim state machine to Visual Mode
        g_Mode = Mode.VISUAL
        
        # 2. Re-anchor Vim's internal origin point to match the selection boundary
        # If the user dragged from bottom-to-top, swap anchors gracefully
        g_VisualModeStartPos = start_sel
        
        # 3. Synchronize the multi-cursor highlight states used by HandleVisualModeChar
        UpdateVisualModeSelection()
        UpdateCursorMode()
        
        print("[vim] Switched to VISUAL mode via Mouse drag")
#------------------------------------------------------------------------
# 10X Callbacks
